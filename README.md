...
- 登录用户的答题图片和笔迹附件存放于私有 R2，并按 180 天保留期清理；访客附件仅保存在当前浏览器的 IndexedDB。
- AI Token 使用用户口令经 PBKDF2 与 AES-GCM 加密后仅保存在本机，解锁后的明文只存在当前浏览器会话和当次 HTTPS 请求中，不写入账户、试卷、数据库或日志。
