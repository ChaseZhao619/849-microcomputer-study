...
export async function GET() {
  try {
    ...
    const sessions = await current.db.select().from(examSessions)
      .where(and(
    ...
    return Response.json({ error: error instanceof Error ? error.message : "试卷读取失败" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    const current = await context();
    if (!current) return Response.json({ error: "请先登录后同步试卷" }, { status: 401 });
    const body = await request.json() as Record<string, unknown>;
    const action = String(body.action || "save");
    if (!["create", "save", "pause", "resume", "submit", "selfScore"].includes(action)) {
      return Response.json({ error: "不支持的试卷操作" }, { status: 400 });
    }
    const id = String(body.id || crypto.randomUUID());
    const questionIds = Array.isArray(body.questionIds) ? body.questionIds.map(Number).filter(Number.isInteger) : [];
    if (!questionIds.length) return Response.json({ error: "试卷题目不能为空" }, { status: 400 });
    const [existing] = await current.db.select({ userId: examSessions.userId }).from(examSessions)
      .where(eq(examSessions.id, id)).limit(1);
    ...
    const status = action === "pause" ? "paused"
      : action === "submit" ? "reviewing"
      : action === "selfScore" ? "completed"
      : "active";
    const values = {
      id,
    ...
    await current.db.insert(examSessions).values(values).onConflictDoUpdate({
    ...
    const [session] = await current.db.select().from(examSessions)
      .where(and(eq(examSessions.id, id), eq(examSessions.userId, current.user.id)))
      .limit(1);
    return Response.json({ ok: true, session });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "试卷保存失败" }, { status: 500 });
  }
}
